...
!----------------------------------------------------------------------
   ...
   case('nemo')
      ...
      create_distribution = create_distrb_nemo(nprocs, work_per_block)
...
!***********************************************************************
!BOP
! !IROUTINE: create_distrb_nemo
! !INTERFACE:

 function create_distrb_nemo(nprocs, workPerBlock) result(newDistrb)
...
!----------------------------------------------------------------------
!
!  set up parameters for NEMO-type decomposition
!
!----------------------------------------------------------------------

   newDistrb%nprocs = nprocs
   newDistrb%numLocalBlocks = 1
...
!----------------------------------------------------------------------
!
!  distribute blocks linearly across processors
!
!----------------------------------------------------------------------
   ...
   do i=1,nblocks_tot
      ...
      if (workPerBlock(globalID) /= 0) then
         newDistrb%blockLocation(globalID) = processor
         newDistrb%blockLocalID (globalID) = 1
         if (processor == my_task + 1) then
            newDistrb%blockGlobalID (1) = globalID
         endif
   ...
   if (nprocs .ne. processor-1) then
      if (my_task == master_task) then
         write(nu_diag,*) ' Number of processors = ',nprocs
         write(nu_diag,*) ' Number of active blocks = ',processor-1
         call abort_ice('ice_distribution: Number of active blocks not equal to number of processors')
      endif
   endif
...
!EOC
 ...
 end function create_distrb_nemo
