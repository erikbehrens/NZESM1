...
CONTAINS
  ...
  !CEB   SUBROUTINE fld_read( kt, kn_fsbc, sd, map, kit, kt_offset )
  SUBROUTINE fld_read( kt, kn_fsbc, sd, map, kit, kt_offset,force_update )
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_read  ***
      !!                   
     ...
     !/CEB
      ...
      !CEB
      force_update2=.FALSE.
      IF(PRESENT(force_update)) force_update2=force_update
      !/CEB
      ...
      IF( MOD( kt-1, kn_fsbc ) == 0 ) THEN         ! update field at each kn_fsbc time-step !
         ...
         DO jf = 1, imf                            ! ---   loop over field   --- !
            ...
            ENDIF   ! read new data?
            !CEB always read the data again
            IF(force_update2 .AND. .NOT. ll_firstcall ) THEN
                !WRITE(numout,*) 'Force update',sd(jf)%nrec_a(:)
