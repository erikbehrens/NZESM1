MODULE sbcflx
 ...
 !CEB  INTEGER , PARAMETER ::   jpfld   = 5   ! maximum number of files to read 
   ...
   INTEGER , PARAMETER ::   jp_evap  = 5   ! index of evaporation-precipation file  !CEB emp->evap
   INTEGER , PARAMETER ::   jp_rain  = 6   ! index of evaporation-precipation file !CEB rain
...
CONTAINS
 ...
 !CEB      TYPE(FLD_N) ::   sn_utau, sn_vtau, sn_qtot, sn_qsr, sn_emp  ! informations about the fields to be read
 !CEB     NAMELIST/namsbc_flx/ cn_dir, sn_utau, sn_vtau, sn_qtot, sn_qsr, sn_emp
      TYPE(FLD_N) ::   sn_utau, sn_vtau, sn_qtot, sn_qsr, sn_evap, sn_rain  ! informations about the fields to be read
      NAMELIST/namsbc_flx/ cn_dir, sn_utau, sn_vtau, sn_qtot, sn_qsr, sn_evap, sn_rain
...
902      IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_flx in configuration namelist', lwp )
   ...
   ! CEB        slf_i(jp_emp ) = sn_emp
         slf_i(jp_evap ) = sn_evap
         slf_i(jp_rain ) = sn_rain
         !/CEB
         !
      ...
      ENDIF
      !CEB
      CALL fld_read( kt, nn_fsbc, sf,force_update=.TRUE. )                            ! input fields provided at the current time-step

      WHERE( sf(jp_qsr)%fnow(:,:,1)  < 0.0 ) sf(jp_qsr)%fnow(:,:,1)=0.0
      WRITE(numout,*)'Min QSR', MINVAL(sf(jp_qsr)%fnow(:,:,1))
      WRITE(numout,*)'Max taux', MINVAL(sf(jp_utau)%fnow(:,:,1))
      !/CEB
...
!CDIR COLLAPSE
         DO jj = 1, jpj                                           ! set the ocean fluxes from read fields
            DO ji = 1, jpi
              ...
              !CEB qns (ji,jj) = sf(jp_qtot)%fnow(ji,jj,1) - sf(jp_qsr)%fnow(ji,jj,1)
              qns (ji,jj) = sf(jp_qtot)%fnow(ji,jj,1) ! heatflux_toyatm_50.nc is the qns so we do not need subtract something
     ! CEB              emp (ji,jj) = sf(jp_emp )%fnow(ji,jj,1)
               emp (ji,jj) = sf(jp_evap )%fnow(ji,jj,1) - sf(jp_rain )%fnow(ji,jj,1)
            END DO
         END DO
...
!CDIR NOVERRCHK
         ...
         IF( nitend-nit000 <= 100 .AND. lwp ) THEN                ! control print (if less than 100 time-step asked)
            ...
            DO jf = 1, jpfld
               ...
               !CEB IF( jf == jp_emp                     )   zfact = 86400.
               IF( jf == jp_evap                     )   zfact = 86400.
               IF( jf == jp_rain                     )   zfact = 86400.
