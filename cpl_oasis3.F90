...
!$AGRIF_DO_NOT_TREAT
   ...
   !CEB
!$AGRIF_DO_NOT_TREAT
    INTEGER, PUBLIC :: localComm  ! opa local communicator CEB
!$AGRIF_END_DO_NOT_TREAT

   !CEB INTEGER, PUBLIC :: localComm 
...
CONTAINS
            ...
            DO jc = 1, srcv(ji)%nct
               DO jm = 1, kcplmodel
                  ...
                  IF ( kcplmodel  > 1 ) THEN
                     ...
                     zclname = 'model'//cli2//'_'//TRIM(zclname)
                  ENDIF
                  WRITE(numout,*) zclname
#if defined key_agrif
                 IF( agrif_fixed() /= 0 ) THEN 
                     zclname=TRIM(Agrif_CFixed())//'_'//TRIM(zclname)
                  WRITE(numout,*) zclname, 'hop'
                  END IF
#endif
   ...
   SUBROUTINE cpl_rcv( kid, kstep, pdata, pmask, kinfo )
      ...
      DO jc = 1, srcv(kid)%nct
         llfisrt = .TRUE.
         !WRITE(numout,*) srcv(kid)%nid(:,:)
         DO jm = 1, srcv(kid)%ncplmodel
            IF( srcv(kid)%nid(jc,jm) /= -1 ) THEN
               !WRITE(numout,*)  srcv(kid)%clname,srcv(kid)%nid(jc,jm), kstep, kinfo
