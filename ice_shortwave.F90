!  SVN:$Id: ice_shortwave.F90 843 2014-10-02 19:54:30Z eclare $
!=======================================================================
!
...
! 2013, ECH: merged with NCAR version, cleaned up
      ...
      real (kind=dbl_kind), public :: &
         ...
         albpondv, & ! melt pond albedo, visible
         albpondi, & ! melt pond albedo, near IR
...
!  Initialize shortwave
      ...
      use ice_state, only: aicen, vicen, vsnon, trcrn, nt_Tsfc, nt_apnd, nt_hpnd, nt_ipnd
...
! Driver for basic solar radiation from CCSM3.  Albedos and absorbed solar.
      ...
      use ice_state, only: tr_pond_topo
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block,ncat), &
         ...
         apndn    , & ! melt pond fraction per category 
         hpndn    , & ! melt pond depth per category 
         ipndn    , & ! melt pond lid thickness per category 
         Tsfcn        ! surface temperature

      real (kind=dbl_kind), dimension (nx_block,ny_block,ncat), &
         intent(inout) :: &
         apeffn       ! effective melt pond fraction per catgeory
      ...
      ! ice, snow and melt pond albedo for each category
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block):: &
         ...
         alvdrnp, & ! visible, direct, melt pond (fraction)
         alidrnp, & ! near-ir, direct, melt pond (fraction)
         alvdfnp, & ! visible, diffuse, melt pond (fraction)
         alidfnp    ! near-ir, diffuse, melt pond (fraction)
         ...
         if (trim(albedo_type) == 'constant') then
            ...
            if (tr_pond_topo) then
                  ...
                  do ij = 1, icells
                     i = indxi(ij)
                     j = indxj(ij)
                     ! Pond only affects albedo if refrozen lid thickness
                     ! is less than 1 cm:
                     if (apndn(i,j,n)*aicen(i,j,n) > puny .and. &
                        ipndn(i,j,n) < ipnd_crit) then
                        fpn(i,j) = apndn(i,j,n)
                     else
                        fpn(i,j) = c0
                     endif
                     if (apndn(i,j,n) > puny) then
                        hpn(i,j) = hpndn(i,j,n)
                     else
                        fpn(i,j) = c0
                        hpn(i,j) = c0
...
! Compute albedos for each thickness category
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block), &
         ...
         vicen   , & ! volume of ice per category
         vsnon   , & ! volume of ice per category
         hpn     , & ! melt pond depth per category
         Tsfcn       ! surface temperature
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block), &
         ...
         alvdfns  , & ! visible, diffuse, snow (fraction)
         alidfns  , & ! near-ir, diffuse, snow (fraction)
         alvdrnp  , & ! visible, direct, melt ponds (fraction)
         alidrnp  , & ! near-ir, direct, melt ponds (fraction)
         alvdfnp  , & ! visible, diffuse, melt ponds (fraction)
         alidfnp  , & ! near-ir, diffuse, melt ponds (fraction)
         alvdrn   , & ! visible, direct, avg   (fraction)
         alidrn   , & ! near-ir, direct, avg   (fraction)
         alvdfn   , & ! visible, diffuse, avg  (fraction)
         alidfn   , & ! near-ir, diffuse, avg  (fraction)
      ...
      real (kind=dbl_kind), parameter :: &
         dT_melt    = c1          , & ! change in temp to give dalb_mlt 
                                     ! albedo change for bare ice
         dT_melt_snow = c1        , & ! change in temp to give dalb_mlt 
                                      ! albedo change for snow
...
!ocl novrec      !Fujitsu
      do ij = 1, icells
         ...
         ! bare ice, temperature dependence (not required if melt ponds are used)
         dTs = Timelt - Tsfcn(i,j)
         fT = min(dTs/dT_melt-c1,c0)
         if (.not. tr_pond_topo) then
         ...
         ! melt pond albedos
         if (tr_pond_topo) then
           if (hpn(i,j) < p001*c4) then  ! < 4mm bare ice albedo
             alvdfnp(i,j) = alvdfni(i,j)
             alidfnp(i,j) = alidfni(i,j)
           elseif (hpn(i,j) > p2) then  ! > 20cm pond albedo         
             alvdfnp(i,j) = albpondv
             alidfnp(i,j) = albpondi
           else  ! linear relationship approx. to Delta-Eddington
             alvdfnp(i,j) = (hpn(i,j)/p2)*albpondv &
                              + (c1-(hpn(i,j)/p2))*alvdfni(i,j)
             alidfnp(i,j) =  (hpn(i,j)/0.2)*albpondi &
                              + (c1-(hpn(i,j)/p2))*alidfni(i,j)
           endif
         endif
         ...
         if (tr_pond_topo) then
           alvdrnp(i,j) = alvdfnp(i,j)
           alidrnp(i,j) = alidfnp(i,j)
         endif
         ...
         ! combine ice/snow albedos with melt pond albedos:
         if (tr_pond_topo) then
           if (hpn(i,j) >= p001*c4) then
             alvdfn(i,j) = alvdfn(i,j)*(c1-fpn(i,j)) + &
                           alvdfnp(i,j)*fpn(i,j)
             alidfn(i,j) = alidfn(i,j)*(c1-fpn(i,j)) + &
                           alidfnp(i,j)*fpn(i,j)
             alvdrn(i,j) = alvdrn(i,j)*(c1-fpn(i,j)) + &
                           alvdrnp(i,j)*fpn(i,j)
             alidrn(i,j) = alidrn(i,j)*(c1-fpn(i,j)) + &
                           alidrnp(i,j)*fpn(i,j)
           endif
         endif
         ...
         albpn(i,j) = awtvdr*alvdrnp(i,j) + awtidr*alidrnp(i,j) &
                    + awtvdf*alvdfnp(i,j) + awtidf*alidfnp(i,j) 
...
!         C. M. Bitz, UW
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block), intent(in) :: &
         ...
         alvdrnp     , & ! visible, direct albedo, melt ponds
         alidrnp     , & ! near-ir, direct albedo, melt ponds
         alvdfnp     , & ! visible, diffuse albedo, melt ponds
         alidfnp         ! near-ir, diffuse albedo, melt ponds
...
! Account for melt pond fraction:
         if (tr_pond_topo) then
           swabsv = swabsv*(c1-fpn(i,j)) &
                  + ( swvdr(i,j)*(c1-alvdrnp(i,j)) &
                  +   swvdf(i,j)*(c1-alvdfnp(i,j)) ) * fpn(i,j)

           swabsi = swabsi*(c1-fpn(i,j)) &
                  + ( swidr(i,j)*(c1-alidrnp(i,j)) &
                  +   swidf(i,j)*(c1-alidfnp(i,j)) ) * fpn(i,j)
         endif

         swabs   = swabsv + swabsi

         if (tr_pond_topo) then
           fswpenvdr = swvdr(i,j) * (c1- &
                    (alvdrni(i,j)*(c1-fpn(i,j)) + alvdrnp(i,j)*fpn(i,j))) * &
                       (c1-asnow) * i0vis
           fswpenvdf = swvdf(i,j) * (c1- &
                    (alvdfni(i,j)*(c1-fpn(i,j)) + alvdfnp(i,j)*fpn(i,j))) * &
                       (c1-asnow) * i0vis
         else
